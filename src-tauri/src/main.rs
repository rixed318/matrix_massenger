...
async fn read_accounts_map(app: &AppHandle) -> Result<HashMap<String, Credentials>, String> {
  let mut store = StoreBuilder::new(app.clone(), STORE_FILE)
      .build()
...
async fn write_accounts_map(app: &AppHandle, map: &HashMap<String, Credentials>) -> Result<(), String> {
  ...
  store.insert(ACCOUNTS_KEY.to_string(), v);
...
fn main() {
  tauri::Builder::default()
    ...
    .invoke_handler(tauri::generate_handler![save_credentials, load_credentials, clear_credentials])
