...
const ChatPage: React.FC<ChatPageProps> = ({ client, onLogout, savedMessagesRoomId }) => {
    ...
    const makePlaceholder = (id: string, roomId: string, body: string) => ({
        id,
        sender: { id: client.getUserId() || 'me', name: 'You', avatarUrl: null },
    ...
    const [outboxItems, setOutboxItems] = useState<Record<string, { attempts: number }>>({});
    ...
    useEffect(() => {
        const unsub = onOutboxEvent(ev => {
            if (ev.kind === 'enqueued') {
                setOutboxItems(prev => ({ ...prev, [ev.item.id]: { attempts: ev.item.attempts } }));
                if (ev.item.roomId === selectedRoomId) {
                    const body = ev.item.content?.body || '';
                    setMessages(prev => [...prev, makePlaceholder(ev.item.id, ev.item.roomId, body)]);
                    setTimeout(() => scrollToBottom('auto'), 50);
                }
            } else if (ev.kind === 'progress') {
                setOutboxItems(prev => ({ ...prev, [ev.id]: { attempts: ev.attempts } }));
            } else if (ev.kind === 'sent') {
                setOutboxItems(prev => { const n = { ...prev }; delete n[ev.id]; return n; });
                // Replace local placeholder id with server event id
                setMessages(prev => prev.map(m => m.id === ev.id ? { ...m, id: ev.serverEventId } : m));
            } else if (ev.kind === 'error') {
                // mark placeholder as error
                setOutboxItems(prev => { const n = { ...prev }; if (n[ev.id]) n[ev.id].error = true; return n; });
            }
        });
        return () => { unsub && unsub(); };
    }, []);
    ...
    useEffect(() => {
        ...
        updateOnline();
        const onl = () => { setIsOffline(false); flushOutbox().catch(()=>{}); };
