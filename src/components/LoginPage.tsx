...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, initialError, savedAccounts: savedAccountsProp, isEmbedded }) => {
  ...
  const removeSavedAccount = useAccountStore(state => state.removeAccount);
  const bootStoredAccounts = useAccountStore(state => state.boot);
  const isBootingAccounts = useAccountStore(state => state.isBooting);
  ...
  const handleSelectSavedAccount = (key: string) => {
    setActiveAccount(key);
  };

  const handleRemoveSavedAccount = async (key: string) => {
    setPendingRemovalKey(key);
    try {
      await removeSavedAccount(key);
    } catch (error) {
      console.error('Failed to remove saved account', error);
      applyError('Не удалось удалить аккаунт. Попробуйте ещё раз.');
    } finally {
  ...
  const handleImportSessions = async () => {
    ...
    try {
      await bootStoredAccounts();
    } catch (error) {
      console.error('Failed to import stored sessions', error);
      applyError('Не удалось импортировать сохранённые сессии.');
    } finally {
    ...
    <div className="mt-3 space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          disabled={isImportingSessions || isBootingAccounts}
          className="self-start rounded-md border border-border-primary px-3 py-1.5 text-xs font-medium text-text-secondary hover:text-text-primary hover:border-accent disabled:opacity-60"
        >
          {isImportingSessions || isBootingAccounts ? 'Импорт...' : 'Импортировать из хранилища'}
        </button>
      </div>
      {savedAccounts.length ? (
        <ul className="space-y-2">
          {savedAccounts.map(account => {
            ...
            const isRemoving = pendingRemovalKey === account.key;
            return (
              <li
                key={account.key}
                className={`rounded-md border px-3 py-2 text-xs transition-colors ${isActive ? 'border-accent bg-accent/10 text-text-primary' : 'border-border-primary text-text-secondary'}`}
              >
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-semibold text-text-primary break-all">{account.user_id}</p>
                    <p className="text-[11px] text-text-secondary break-all">{account.homeserver_url}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => handleSelectSavedAccount(account.key)}
                      disabled={isActive || isRemoving}
                      className={`rounded-md px-3 py-1.5 text-xs font-medium ${isActive ? 'bg-accent text-text-inverted' : 'border border-border-primary text-text-secondary hover:text-text-primary hover:border-accent'}`}
                    >
        ...
        <p className="text-xs text-text-secondary">Нет сохранённых аккаунтов. Войдите, чтобы сохранить новую сессию.</p>
      )}
    </div>
  );
