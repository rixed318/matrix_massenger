...
import { themes, spacing, radii, typography, controls, ThemeTokens } from './index.js';

type ThemeEntry = [selector: string, theme: ThemeTokens<string, number>];

const colorVariableMap = (theme: ThemeTokens<string, number>) => ({
...
const mapScale = (scale: Record<string, number>) => Object.fromEntries(
  Object.entries(scale).map(([key, value]) => [key, toPx(value)]),
);
