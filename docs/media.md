...
Matrix Messenger поддерживает формат сторис, синхронизированный между устройствами через Matrix account data.

- **Типы медиа.** В одну историю можно добавить фотографию или короткое видео. Файл загружается в Matrix media store, а в событии `econix.story` сохраняется метаданные (тип, длительность, миниатюра, подпись).
- **Просмотры и реакции.** Каждый просмотр записывается в событие `econix.story.read`, а счетчики реакций и просмотров пересчитываются на лету. Нажмите на быстрое эмодзи в просмотрщике, чтобы оставить реакцию.
- **Локальный кэш.** Клиент сохраняет список историй в IndexedDB (в браузере) или в Tauri Store (в десктоп-приложении), чтобы история была доступна даже до полной синхронизации.
- **UI.** Лента сторис отображается поверх списка чатов, отдельный просмотрщик поддерживает свайпы/клавиши стрелок, быстрые реакции и показывает количество просмотров.
